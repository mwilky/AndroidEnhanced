...
import com.mwilky.androidenhanced.HookedClasses.Companion.WINDOW_MANAGER_FUNCS_CLASS
...
class Buttons {
    ...
    companion object {
        ...
        private var PhoneWindowManagerClass: Class<*>? = null

        fun initLauncher(classLoader: ClassLoader?) {

            // Double tap launcher to sleep
            // Register the observer
            findAndHookMethod(LAUNCHER_APPLICATION_CLASS,
                classLoader,
                "onCreate",
                object : XC_MethodHook() {
                    override fun afterHookedMethod(param: MethodHookParam) {
                        ...
                        registerBroadcastReceiver(
                            mContext, doubleTapToSleepLauncher, param.thisObject.toString(), false
                        )
            ...
            hookAllConstructors(
                findClass(WORKSPACE_TOUCH_LISTENER_CLASS, classLoader),
                object : XC_MethodHook() {
                    override fun afterHookedMethod(param: MethodHookParam) {
        ...
        fun init(classLoader: ClassLoader?) {
            ...
            PhoneWindowManagerClass = findClass(
                PHONE_WINDOW_MANAGER_CLASS, classLoader
            )

            // Hooked methods
            // Method to register the receivers + set PhoneWindowManager object
            findAndHookMethod(PHONE_WINDOW_MANAGER_CLASS,
                classLoader,
                "init",
                Context::class.java,
                WINDOW_MANAGER_FUNCS_CLASS,
                object : XC_MethodHook() {
                    override fun afterHookedMethod(param: MethodHookParam) {
                        ...
                        registerBroadcastReceiver(
                            mContext, torchPowerScreenOff, param.thisObject.toString(), false
                        )
                        registerBroadcastReceiver(
                            mContext, volKeyMediaControl, param.thisObject.toString(), false
                        )
                        //Register this in this class so we don't need to rehook in Lockscreen.kt
                        registerBroadcastReceiver(
                            mContext, disableLockscreenPowerMenu, param.thisObject.toString(), false
                        )
                        //Register this in this class
                        registerBroadcastReceiver(
                            mContext, muteScreenOnNotifications, param.thisObject.toString(), false
                        )

                        registerBroadcastReceiver(
                            mContext, disableCameraScreenOff, param.thisObject.toString(), false
                        )
            ...
            findAndHookMethod(
                ...
                object : XC_MethodHook() {
                    override fun afterHookedMethod(param: MethodHookParam) {
                        ...
                        // Register broadcast receiver to receive values
                        registerBroadcastReceiver(
                            mContext, torchAutoOffScreenOn, param.thisObject.toString(), false
                        )
            ...
            findAndHookMethod(PHONE_WINDOW_MANAGER_POWER_KEY_RULE_CLASS,
                classLoader,
                "onLongPress",
            ...
            // Vol Key Wake
            // Handle the actions
            findAndHookMethod(PHONE_WINDOW_MANAGER_POLICY_HANDLER_CLASS,
                classLoader,
                "handleMessage",
                Message::class.java,
                object : XC_MethodHook() {
            ...
            findAndHookMethod(PHONE_WINDOW_MANAGER_CLASS,
                ...
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam) {
                        ...
                        val telecomManager =
                            callMethod(param.thisObject, "getTelecommService") as TelecomManager
            ...
            // send the action if torch is on and not woken by lift wake reason
            findAndHookMethod(POWER_MANAGER_SERVICE_CLASS,
                classLoader,
            ...
            // Torch auto off function
            // Perform the toggle
            findAndHookMethod(POWER_MANAGER_SERVICE_POWER_MANAGER_HANDLER_CALLBACK_CLASS,
                classLoader,
                "handleMessage",
                Message::class.java,
                object : XC_MethodHook() {
                ...
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam) {
                        ...
                        val dreamManager: Any = callStaticMethod(
                            PhoneWindowManagerClass, "getDreamManager"
                        )
            ...
            // Disable camera gesture when screen off
            findAndHookMethod(GESTURE_LAUNCHER_SERVICE_CLASS,
                classLoader,
                "handleCameraGesture",
                Boolean::class.javaPrimitiveType,
                Int::class.javaPrimitiveType,
